...
export function encodeToUrl(state) {
  const compact = {
    ...
    k: state.soundKit,
    rv: state.reverbMix,
    ct: state.compThreshold,
    cr: state.compRatio,
    vp: state.voiceParams,
    n: state.bars,
  };
  TRACKS.forEach(t => {
...
export function decodeFromUrl(hash) {
  try {
    ...
    return {
      ...
      soundKit: compact.k || 'standard',
      reverbMix: compact.rv || 0,
      compThreshold: compact.ct || -12,
      compRatio: compact.cr || 4,
      voiceParams: compact.vp || null,
      bars,
    };
  } catch {
    return null;

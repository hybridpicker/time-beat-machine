...
export function triggerCymbal(ctx, dest, time, velocity = 1, params) {
  ...
  if (isAcoustic(params)) {
    ...
    const stickHp = ctx.createBiquadFilter();
    stickHp.type = 'highpass';
    stickHp.frequency.value = (1700 + tone * 450) * tune;
    stickHp.Q.value = 0.12;
    const stickBp = ctx.createBiquadFilter();
    stickBp.type = 'bandpass';
    stickBp.frequency.value = (2600 + texture * 700) * tune;
    stickBp.Q.value = 0.55;
    const stickGain = ctx.createGain();
    stickGain.gain.setValueAtTime((0.075 + texture * 0.018) * v, time);
    stickGain.gain.exponentialRampToValueAtTime(0.001, time + 0.022);
    stick.connect(stickHp).connect(stickBp).connect(stickGain).connect(dest);
    stick.start(time);
    ...
    washHp.frequency.value = (1900 + tone * 650) * tune;
    washHp.Q.value = 0.1;
    const washLp = ctx.createBiquadFilter();
    washLp.type = 'lowpass';
    washLp.frequency.value = (6900 + texture * 1100) * tune;
    washLp.Q.value = 0.18;
    const washShelf = ctx.createBiquadFilter();
    washShelf.type = 'highshelf';
    washShelf.frequency.value = 5600 * tune;
    washShelf.gain.value = -12 + texture * 2;
    const washGain = ctx.createGain();
    washGain.gain.setValueAtTime((0.055 + texture * 0.025) * v, time + 0.006);
    washGain.gain.exponentialRampToValueAtTime((0.03 + texture * 0.018) * v, time + 0.11);
    washGain.gain.exponentialRampToValueAtTime(0.001, time + (1.45 + texture * 0.95) * decay);
    wash.connect(washHp).connect(washLp).connect(washShelf).connect(washGain).connect(dest);
    wash.start(time + 0.006);
    wash.stop(time + (1.9 + texture * 1.3) * decay);
    return;
  }
